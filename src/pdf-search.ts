 ...
 */
function parseFilename(filename: string): { manufacturer?: string; model?: string; type?: string } {
 ...
 */
export function getAllManuals(): PDFManual[] {
  try {
 ...
 */
export function findManual(manufacturer?: string, model?: string): PDFManual | null {
...
async function searchPDFContent(
...
): Promise<{ pageNumber: number; content: string; relevance: number }[]> {
  const { text } = await extractPDFText(pdfPath);
  const lines = text.split('\n');
  const results: { pageNumber: number; content: string; relevance: number }[] = [];
  ...
  const uniqueResults = results
    ...
    .filter((result, index, self) =>
      index === self.findIndex(r => r.pageNumber === result.pageNumber)
    )
...
export async function getManualContent(
  manufacturer: string,
  model?: string
): Promise<{ filename: string; content: string; pageCount: number } | null> {
